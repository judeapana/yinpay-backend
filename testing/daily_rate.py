import json
import unittest

import flask_unittest

from yinpay import create_app


class DailyRateTestCase(flask_unittest.ClientTestCase):
    args = {"selector": "bdf7fe91-20d0-4820-a930-6a7d93837b1d"}
    app = create_app()
    token = "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.._DOD3BOZ-nXi40m0A3xJcdrz6dCfLtDGB9zcz23FpHA"

    def test_get(self, client):
        request = client.get('http://127.0.0.1:5000/daily-rate/', query_string=self.args,
                             headers={'Authorization': self.token})
        # print(request.json)

    def test_post(self, client):
        data = json.dumps(dict(
            main_amount="109",
            notes="",
            period_id="560c3029-9456-43bb-847f-a2edb728d6e5",
            user_meta_id="014c7589-7ee7-4e86-83ef-82c23bb320c3",

        ))
        request = client.post('http://127.0.0.1:5000/daily-rate/', query_string=self.args,
                              headers={'Authorization': self.token}, data=data,
                              content_type='application/json')
        # print(request.json)

    def test_get_pk(self, client):
        request = client.get('http://127.0.0.1:5000/daily-rate/7630ea21-f063-40a7-a574-08024a93bef1', query_string=self.args,
                             headers={'Authorization': self.token})
        # print(request.json)

    def test_put_pk(self, client):
        data = json.dumps(dict(
            main_amount="200",
            notes="",
            period_id="560c3029-9456-43bb-847f-a2edb728d6e5",
            user_meta_id="014c7589-7ee7-4e86-83ef-82c23bb320c3",

        ))
        request = client.put('http://127.0.0.1:5000/daily-rate/7630ea21-f063-40a7-a574-08024a93bef1', query_string=self.args,
                             headers={'Authorization': self.token}, data=data,
                             content_type='application/json')
        # print(request.json)

    def test_delete_pk(self, client):
        request = client.delete('http://127.0.0.1:5000/daily-rate/7630ea21-f063-40a7-a574-08024a93bef1',
                                query_string=self.args,
                                headers={'Authorization': self.token})
        # print(request.json)


if __name__ == "__main__":
    unittest.main()
