...
class BankDetailTestCase(flask_unittest.ClientTestCase):
    ...
    def test_get(self, client):
        request = client.get('http://127.0.0.1:5000/bank-detail/', query_string=self.args,
                             headers={'Authorization': self.token})
        # print(request.json)

    def test_post(self, client):
        data = json.dumps(dict(
            bank_id="ca91380a-dc92-4910-986d-b3f6dacbae3b",
            currency="GHS",
            no="4040409330012120",
            user_meta_id="014c7589-7ee7-4e86-83ef-82c23bb320c3",
            branch="Bolgatanga",
        ))
        request = client.post('http://127.0.0.1:5000/bank-detail/', query_string=self.args,
                              headers={'Authorization': self.token}, data=data,
                              content_type='application/json')
        # print(request.json)

    def test_get_pk(self, client):
        request = client.get('http://127.0.0.1:5000/bank-detail/2b1a03e9-8fc0-491c-b681-a7277baff6b4',
    ...
    def test_put_pk(self, client):
        ...
        request = client.put('http://127.0.0.1:5000/bank-detail/2b1a03e9-8fc0-491c-b681-a7277baff6b4',
                             query_string=self.args,
                             headers={'Authorization': self.token}, data=data,
                             content_type='application/json')
        # print(request.json)
    ...
    def test_delete_pk(self, client):
        request = client.delete('http://127.0.0.1:5000/bank-detail/2b1a03e9-8fc0-491c-b681-a7277baff6b4',
                                query_string=self.args,
                                headers={'Authorization': self.token})
        # print(request.json)
