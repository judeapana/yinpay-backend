...
class DeductionGroupTestCase(flask_unittest.ClientTestCase):
    ...
    def test_get(self, client):
        request = client.get('http://127.0.0.1:5000/deduction-group/', query_string=self.args,
    ...
    def test_post(self, client):
        ...
        request = client.post('http://127.0.0.1:5000/deduction-group/', query_string=self.args,
                              headers={'Authorization': self.token}, data=data,
                              content_type='application/json')
        # print(request.json)

    def test_get_pk(self, client):
        request = client.get('http://127.0.0.1:5000/deduction-group/6fb71c93-ec40-4fda-97d5-241de3d1155a',
    ...
    def test_put_pk(self, client):
        data = json.dumps(dict(
            name='Car Allowance 33',
            notes="Test Bank",
            personnel_group_id="1f22ac4a-d341-4775-b6b2-82de66223a41",
        ))
        request = client.put('http://127.0.0.1:5000/deduction-group/6503f210-9987-4c0d-9a20-f15ff09a7766',
                             query_string=self.args,
                             headers={'Authorization': self.token}, data=data,
                             content_type='application/json')
        print(request.json)
    ...
    def test_delete_pk(self, client):
        request = client.delete('http://127.0.0.1:5000/deduction-group/6fb71c93-ec40-4fda-97d5-241de3d1155a',
                                query_string=self.args,
                                headers={'Authorization': self.token})
        # print(request.json)
