import json
import unittest

import flask_unittest

from yinpay import create_app


class EarningGroupTestCase(flask_unittest.ClientTestCase):
    args = {"selector": "bdf7fe91-20d0-4820-a930-6a7d93837b1d"}
    app = create_app()
    token = "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..sfg255YvJlYU-erwrAKip5In2-cIL45l4TzDu7SdPu4"

    def test_get(self, client):
        request = client.get('http://127.0.0.1:5000/earning-group/', query_string=self.args,
                             headers={'Authorization': self.token})
        # print(request.json)

    def test_post(self, client):
        data = json.dumps(dict(
            name='Car Allowance',
            notes="Test Bank",
            personnel_group_id="1f22ac4a-d341-4775-b6b2-82de66223a41",
        ))
        request = client.post('http://127.0.0.1:5000/earning-group/', query_string=self.args,
                              headers={'Authorization': self.token}, data=data,
                              content_type='application/json')
        # print(request.json)

    def test_get_pk(self, client):
        request = client.get('http://127.0.0.1:5000/earning-group/9c96fc03-afa0-4303-9b9b-92ad0901a22e',
                             query_string=self.args,
                             headers={'Authorization': self.token})
        # print(request.json)

    def test_put_pk(self, client):
        data = json.dumps(dict(
            name='Car Allowance22',
            notes="Test Bank",
            personnel_group_id="1f22ac4a-d341-4775-b6b2-82de66223a41",
        ))
        request = client.put('http://127.0.0.1:5000/earning-group/9c96fc03-afa0-4303-9b9b-92ad0901a22e',
                             query_string=self.args,
                             headers={'Authorization': self.token}, data=data,
                             content_type='application/json')
        # print(request.json)

    def test_delete_pk(self, client):
        request = client.delete('http://127.0.0.1:5000/earning-group/9c96fc03-afa0-4303-9b9b-92ad0901a22e',
                                query_string=self.args,
                                headers={'Authorization': self.token})
        print(request.json)


if __name__ == "__main__":
    unittest.main()
