import json
import unittest

import flask_unittest

from yinpay import create_app


class PeriodTestCase(flask_unittest.ClientTestCase):
    args = {"selector": "bdf7fe91-20d0-4820-a930-6a7d93837b1d"}
    app = create_app()
    token = "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.._DOD3BOZ-nXi40m0A3xJcdrz6dCfLtDGB9zcz23FpHA"

    def test_get(self, client):
        request = client.get('http://127.0.0.1:5000/period/', query_string=self.args,
                             headers={'Authorization': self.token})
        # print(request.json)

    def test_post(self, client):
        data = json.dumps(dict(
            name="June 2020",
            notes="June month",
            month="01-01-2020"
        ))
        request = client.post('http://127.0.0.1:5000/period/', query_string=self.args,
                              headers={'Authorization': self.token}, data=data,
                              content_type='application/json')
        # print(request.json)

    def test_get_pk(self, client):
        request = client.get('http://127.0.0.1:5000/period/8e00cb8b-e72f-4d07-a76c-3ae2899d0a8f',
                             query_string=self.args,
                             headers={'Authorization': self.token},
                             content_type='application/json')
        # print(request.json)

    def test_put_pk(self, client):
        data = json.dumps(dict(
            name="June 2020",
            notes="June month",
            month="06-06-1999"
        ))
        request = client.put('http://127.0.0.1:5000/period/8e00cb8b-e72f-4d07-a76c-3ae2899d0a8f',
                             query_string=self.args,
                             headers={'Authorization': self.token}, data=data,
                             content_type='application/json')
        print(request.json)

    def test_delete_pk(self, client):
        request = client.delete('http://127.0.0.1:5000/period/8e00cb8b-e72f-4d07-a76c-3ae2899d0a8f',
                                query_string=self.args,
                                headers={'Authorization': self.token},
                                content_type='application/json')
        # print(request.json)


if __name__ == "__main__":
    unittest.main()
