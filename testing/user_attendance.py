import json
import unittest

import flask_unittest

from yinpay import create_app


class UserAttendanceTestCase(flask_unittest.ClientTestCase):
    args = {"selector": "bdf7fe91-20d0-4820-a930-6a7d93837b1d"}
    app = create_app()
    token = "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..sfg255YvJlYU-erwrAKip5In2-cIL45l4TzDu7SdPu4"

    def test_get(self, client):
        request = client.get('http://127.0.0.1:5000/user-attendance/', query_string=self.args,
                             headers={'Authorization': self.token})
        # print(request.json)

    def test_post(self, client):
        data = json.dumps(dict(
            user_meta_id="014c7589-7ee7-4e86-83ef-82c23bb320c3",
            attype="Absent",
            attendance_id="2117463d-f3be-4f13-8531-ebf52f81ece3",
            time="01-01-2019 7:04:44 pm",
            type="Clock In"
        ))
        request = client.post('http://127.0.0.1:5000/user-attendance/', query_string=self.args,
                              headers={'Authorization': self.token}, data=data,
                              content_type='application/json')
        # print(request.json)

    def test_get_pk(self, client):
        request = client.get('http://127.0.0.1:5000/user-attendance/5bfe295e-6d8e-4004-85f4-af2c1fdee669',
                             query_string=self.args,
                             headers={'Authorization': self.token})
        # print(request.json)

    def test_put_pk(self, client):
        data = json.dumps(dict(
            user_meta_id="014c7589-7ee7-4e86-83ef-82c23bb320c3",
            attype="Present",
            attendance_id="2117463d-f3be-4f13-8531-ebf52f81ece3",
            time="01-01-2019 7:04:44 pm",
            type="Clock In"
        ))
        request = client.put('http://127.0.0.1:5000/user-attendance/5bfe295e-6d8e-4004-85f4-af2c1fdee669',
                             query_string=self.args,
                             headers={'Authorization': self.token}, data=data,
                             content_type='application/json')
        # print(request.json)

    def test_delete_pk(self, client):
        request = client.delete('http://127.0.0.1:5000/user-attendance/5bfe295e-6d8e-4004-85f4-af2c1fdee669',
                                query_string=self.args,
                                headers={'Authorization': self.token})
        # print(request.json)


if __name__ == "__main__":
    unittest.main()
