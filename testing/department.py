import json
import unittest

import flask_unittest

from yinpay import create_app


class DepartmentTestCase(flask_unittest.ClientTestCase):
    args = {"selector": "bdf7fe91-20d0-4820-a930-6a7d93837b1d"}
    app = create_app()
    token = "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.._DOD3BOZ-nXi40m0A3xJcdrz6dCfLtDGB9zcz23FpHA"

    def test_get(self, client):
        request = client.get('http://127.0.0.1:5000/department/', query_string=self.args,
                             headers={'Authorization': self.token})
        self.assertEqual(request.status, 200)

    def test_post(self, client):
        request = client.post('http://127.0.0.1:5000/department/', query_string=self.args,
                              headers={'Authorization': self.token}, data=json.dumps(dict(name='Department')),
                              content_type='application/json')

        self.assertEqual(request.status, 200)

    def test_get_pk(self, client):
        request = client.get('http://127.0.0.1:5000/department/83e5a479-99a0-4e73-875d-737f6fecf63d',
                             query_string=self.args,
                             headers={'Authorization': self.token})
        self.assertEqual(request.status, 200)

    def test_put_pk(self, client):
        request = client.put('http://127.0.0.1:5000/department/83e5a479-99a0-4e73-875d-737f6fecf63d',
                             query_string=self.args,
                             headers={'Authorization': self.token}, data=json.dumps(dict(name='New Department')),
                             content_type='application/json')
        self.assertEqual(request.status, 200)

    def test_delete_pk(self, client):
        request = client.delete('http://127.0.0.1:5000/department/83e5a479-99a0-4e73-875d-737f6fecf63d',
                                query_string=self.args,
                                headers={'Authorization': self.token})
        self.assertEqual(request.status, 200)


if __name__ == '__main__':
    unittest.main()
